...
def get_label_from_node(phylox_network, node):
    from phylox.constants import LABEL_ATTR
    return phylox_network.nodes[node][LABEL_ATTR]


def get_node_from_label(phylox_network, label):
    return phylox_network.labels[label][0]


def get_taxa_label(phylox_network):
    """
    ...
    """
    import re
    newick_str = phylox_network.newick().strip()
...
def sister_node_label(phylox_network, node_label):
    ...
    node = get_node_from_label(phylox_network, node_label)

    # Get its parent
    parent = next(phylox_network.predecessors(node), None)
    if parent is None:
        raise KeyError(f"Taxon {node_label} has no parent (it may be a root).")

    # Find its sister (any other child of the parent)
    sister = None
    for child in phylox_network.successors(parent):
        if child != node:
            sister = child
            break
    ...
    return get_label_from_node(phylox_network, sister)


def is_asymmetric_quartet(quartet_tree):
    """Returns True if quartet_tree is asymmetric quartet and False otherwise."""

    def suppress_degree2_node(q_tree):
        """Suppress all degree-2 nodes (in-degree 1 and out-degree 1)"""
        from phylox import suppress_node
        degree2_nodes = [node for node in q_tree.nodes
                         if q_tree.in_degree(node) == 1 and q_tree.out_degree(node) == 1]
        for node in degree2_nodes:
            suppress_node(q_tree, node)

    # Work on a copy so we don't modify the original
    quartet = quartet_tree.copy()

    # 1. Suppress all degree-2 nodes FIRST
    suppress_degree2_node(quartet)
    ...
    isAsymm = any(child in quartet.leaves for child in quartet.successors(root))
    return isAsymm


def sorting_asymmetric_quartet(asymmetric_quart):
    """
    Get the taxa permutation to ladderize an input asymmetric_quart into a sorted
    ...
    """
    # Borrow ladderize() of Dendropy package to sort asymmetric quartet
    import dendropy

    unsorted_taxa_order = get_taxa_label(asymmetric_quart)

    dendro_tree = dendropy.Tree.get(data=asymmetric_quart.newick(), schema="newick", rooting="default-rooted")

    # Sort the asymmetric quartet in ascending order using 'ladderize'
    dendro_tree.ladderize(ascending=True)  # e.g. Sort (A,((B,C),D)) into (A,(D,(B,C)))
    sorted_taxa_order = [leaf.taxon.label for leaf in dendro_tree.leaf_nodes()]

    # Return the taxa permutation of input asymmetric_quart
    return [unsorted_taxa_order.index(x) for x in sorted_taxa_order]


def ladderize_network(phylox_network, ascending=False):
    """
    ...
    Returns:
    - phylox.DiNetwork: A new network with ordered edges.
    """
    import networkx as nx
    import phylox

    if not nx.is_directed_acyclic_graph(phylox_network):
    ...
    descendant_leaves = {n: set() for n in phylox_network.nodes()}
    for leaf in leaves:
        descendant_leaves[leaf].add(leaf)

    for node in reversed(list(nx.topological_sort(phylox_network))):
        for child in phylox_network.successors(node):
    ...
    for node in phylox_network.nodes():
        children = list(phylox_network.successors(node))
...
def retain_taxa_with_labels(labeled_network, taxa_labels):
    ...
    def prune_leaf_branch(pruned_tree, leaf):
        ...
        def get_need_remove_nodes(pruned_tree, node):
            ...
            while True:
                ...
                outDegree = pruned_tree.out_degree(parent)
        ...
        need_remove = get_need_remove_nodes(pruned_tree, leaf)
        pruned_tree.remove_nodes_from(need_remove)

    # Work on a copy so we don't modify the original
    pruned_tree = labeled_network.copy()

    # First, we find remove_leaves
    all_leaves = list(pruned_tree.leaves)
    retain_leaves = [get_node_from_label(pruned_tree,label) for label in taxa_labels]
    ...
    for leaf in remove_leaves:
        prune_leaf_branch(pruned_tree, leaf)

    return pruned_tree


def get_displayed_trees(phylox_network):
    """
    ...
    reticulations = sorted(list(phylox_network.reticulations), key=get_retic_number)
    h = len(reticulations)

    # Base case: No reticulations → return the network as the only displayed tree
    if h == 0:
        return [phylox_network.copy()]
    ...
    for bits in itertools.product([0, 1], repeat=h):
        ...
        for i, bit in enumerate(bits):
            ...
            if disp_tree.in_degree(parent_node) == 1 and disp_tree.out_degree(parent_node) == 1:
                suppress_node(disp_tree, parent_node)
            ...
            if disp_tree.in_degree(retic_node) == 1 and disp_tree.out_degree(retic_node) == 1:
                suppress_node(disp_tree, retic_node)
...
def get_displayedTreeNewick_gammaID(phylox_network, major_tree):
    """
    ...
    gamma_id=1 means weight=gamma_1. gamma_id=-1 means weight=1-gamma_1.
    gamma_id=[1,-2] means weight=gamma_1*(1-gamma_2)
    """
    import itertools
    import re
    ...
    def remove_hybrid_branch(phylox_network, in_edge):
    ...
    def get_leaf_labels_below_node(phylox_network, node):
        ...
        while stack:
            ...
            for curr_node in node_list:
                children = list(phylox_network.successors(curr_node))
                if not children:
                    try:
                        leaf_labels.add(get_label_from_node(phylox_network, curr_node))
                    except KeyError:
                        pass
                else:
    ...
    def suppress_all_degree2_nodes(tree):
        """Safely cleans up the leftover degree-2 nodes after edge removal."""
        from phylox import suppress_node
        degree2_nodes = [node for node in tree.nodes
                         if tree.in_degree(node) == 1 and tree.out_degree(node) == 1]
        for node in degree2_nodes:
            suppress_node(tree, node)
    ...
    for bits in itertools.product([0, 1], repeat=h):
        ...
        for i, bit in enumerate(bits):
            in_edge = next(itertools.islice(disp_tree.in_edges(reticulations[i]), bit, None))
            remove_hybrid_branch(disp_tree, in_edge)

            leaf_labels_disp = get_leaf_labels_below_node(disp_tree, next(disp_tree.predecessors(reticulations[i])))
            hybrid_node = get_node_from_label(major_tree, get_label_from_node(labeled_network, reticulations[i]))
            leaf_labels_majr = get_leaf_labels_below_node(major_tree, next(major_tree.predecessors(hybrid_node)))
...
def get_weighted_displayed_tree(phylox_network, major_tree):
    ...
    def remove_hybrid_branch(phylox_network, in_edge):
        """Don't suppress the degree 2 node or it brings problem to parse zipped_data_net"""
        # from phylox import suppress_node
        parent, child = in_edge
        # Remove one in-edge
        phylox_network.remove_edge(parent, child)
    ...
    def get_leaf_labels_below_node(phylox_network, node):
        ...
        while stack:
            ...
            for curr_node in node_list:
                children = list(phylox_network.successors(curr_node))
                # If no children → it's a leaf_node
                if not children:
                    try:    # Try to get its label (may fail if leaf_node has no label)
                        leaf_labels.add(get_label_from_node(phylox_network, curr_node))
                    except KeyError:
                        pass # If the leaf_node has no label (rare), ignore it.
                else:
                    # Continue Depth-First Search (DFS) stack
                    stack.extend(children)
        return leaf_labels
    ...
    for bits in itertools.product([0, 1], repeat=h):
        ...
        all_dispTree.append((disp_tree, gamma_id))

    return all_dispTree


def label_speciation_time_idx(phylox_network):
    """
    ...
    internal_nodes = [node for node in phylox_network.nodes if phylox_network.out_degree[node] > 0]

    n = len(phylox_network.leaves)          # number of taxa
    h = len(phylox_network.reticulations)   # number of hybrids
    J = n + h - 1                           # total number of tau parameters

    label_index = J

    for node in internal_nodes:
        phylox_network.nodes[node][LABEL_ATTR] = label_index
        label_index -= 1
...
def get_newick_format(labeled_network):
    ...
    root = list(labeled_network.roots)[0]
    if labeled_network.out_degree(root) == 1:
        root = next(labeled_network.successors(root))

    # --- Step 2: Recursive function
    def node_to_newick(node):
        # Leaf: return its taxon label
        if node in labeled_network.leaves:
            return str(labeled_network.nodes[node]["label"])

        # Internal node: process children
        children = list(labeled_network.successors(node))
...
def erase_internal_labels(phylox_network):
    """
    ...
    for node in internal_nodes:
        ...
        if LABEL_ATTR in phylox_network.nodes[node]:
            del phylox_network.nodes[node][LABEL_ATTR]


def get_quartet_parameter_idx(labeled_quartet):
    """
    Extracts the speciation time indices of the quartet network.
    """
    # If the out-degree of the root node is one, do not include it as speciation_nodes
    root = list(labeled_quartet.roots)[0]
    if labeled_quartet.out_degree(root) == 1:
        # If root is unary, speciation_nodes = [all degree-3 nodes]
        speciation_nodes = [node for node in labeled_quartet.nodes if labeled_quartet.degree[node] == 3]
    else:
        # If root is binary, speciation_nodes = root + [all degree-3 nodes]
        speciation_nodes = [root] + [node for node in labeled_quartet.nodes if labeled_quartet.degree[node] == 3]

    # Get the speciation time index labeled on speciation_nodes by pre-order traversal
    param_idx = np.array([get_label_from_node(labeled_quartet, node) for node in speciation_nodes])
...
def print_tree(phylox_tree):
    ...
    erase_internal_labels(clone_tree)
...
####################################################################################################
## Break displayed trees into subquartet and save them as (param_idx, isAsymm, taxaPerm, gamma_id) ##
####################################################################################################

def get_all_quartet_features(phylox_network, major_tree):
    """
    For each combination of 4 taxa, get the quartet features (parameter indices: param_idx, quartet types: isAsymm,
    taxa permutations: taxaPerm, and gamma id's: gamma_id) of the sub-quartets pulled from each displayed tree. If
    sub-quartets have the same param_idx for a combination of 4 taxa, we only save one quartet feature. Otherwise,
    we store all quartet features as a list of param_idx, of isAsymm, of taxaPerm, and of gamma_id that correspond
    to the combination of 4 taxa. These quartet features are used to compute the true probabilities of the quartet.

    The quartet features are stored in an order according to "combinations" of "itertools" in get_quartet_subtrees.
    ...
    taxa_labels = get_taxa_label(phylox_network)
    four_taxa_combs = combinations(taxa_labels, 4)

    # Get all displayed trees with gamma_id's used to compute weight
    all_disp_tree = get_weighted_displayed_tree(phylox_network, major_tree)
    ...
    for taxa_comb in four_taxa_combs:
        ...
        # dict_isAsymm = defaultdict(bool)
        # dict_taxaPerm = defaultdict(list)
        # dict_gamma_id = defaultdict(list)
        dict_isAsymm = {}
        dict_taxaPerm = {}
        dict_gamma_id = {}
        taxa_list = list(taxa_comb) # Pre-convert the tuple to a list once per quartet

        for disp_tree, gamma_id in all_disp_tree:
            # Prune the displayed tree to keep only the current set of four taxa
            labeled_quartet = retain_taxa_with_labels(disp_tree, taxa_list)

            # Erase labels on internal nodes for quartet sorting
            quartet_network = labeled_quartet.copy()
            erase_internal_labels(quartet_network)

            # Use param_idx as key to store other features
            dict_key = tuple(get_quartet_parameter_idx(labeled_quartet))
            isAsymm = is_asymmetric_quartet(labeled_quartet)

            dict_isAsymm[dict_key] = isAsymm
            # Only sort the asymmetric quartet
            dict_taxaPerm[dict_key] = sorting_asymmetric_quartet(quartet_network) if isAsymm else [0,1,2,3]
            dict_gamma_id[dict_key] = gamma_id

        # Extract collapsed result from dictionary
        all_param_idx.append( np.array(list(dict_isAsymm.keys())) )
        all_isAsymm.append( np.array(list(dict_isAsymm.values())) )
        all_taxaPerm.append( np.array(list(dict_taxaPerm.values())) )
        all_gamma_id.append( np.array(list(dict_gamma_id.values())) )

    return all_param_idx, all_isAsymm, all_taxaPerm, all_gamma_id


###########################################################################
## Tools for manipulating parameters (parameters, gamma_weights) ##
###########################################################################
...
def get_tau_theta(tree_parameters, tau_idx):
    """
    ...
    """
    theta = tree_parameters[-1]
    tau = tree_parameters[:-1]  # the last element is theta
    J = len(tree_parameters) - 1  # total number of tau parameters
    tau_idx_0base = J - np.array(tau_idx)  # adjust for 0-based indexing of tau

    # Output in tau1, tau2, tau3 for convenience to input in getTrueProbs functions.
    return [tau[i] for i in tau_idx_0base[::-1]] + [theta]

def get_gamma_weight(gamma_parameters, gamma_id):
    """
    Get Gamma_t.
    gamma_id=[] -> 1
    gamma_id=1  -> gamma_1
    ...
    """
    if gamma_id.size == 0:          # returns 1 if gamma_id is empty
        return 1
    elif isinstance(gamma_id, int): # Normalize to list
        gamma_id = [gamma_id]

    Gamma_t = 1.0
    for idx in gamma_id:
        i = abs(idx) - 1  # convert to 0-based index
        g = gamma_parameters[i]
        Gamma_t *= g if idx > 0 else (1 - g)

    return Gamma_t

def get_gamma_1st_deriv(gamma_parameters, gamma_id):
    """
    Get ∂Γ_t / ∂γ_i (dGt_dg)
    gamma_id=1  -> 1
    gamma_id=-1 -> -1
    ...
    for i,idx in enumerate(gamma_id):
        ...
        weight = get_gamma_weight(gamma_parameters, new_gamma_id)
        dGt_dg[i] = np.sign(idx) * weight

    return dGt_dg

def get_gamma_2nd_deriv(gamma_parameters, gamma_id):
    """
    ...
    if num_gam == 1:
        ...
        dGt_dgg = np.matrix(np.zeros((num_gam, num_gam)))
...
def get_sitePattern_relationship(taxaPerm, inverse=False):
    """
    ...
    if inverse:
        return dict_relationship[tuple(np.argsort(taxaPerm))]
    else:
        return dict_relationship[tuple(taxaPerm)]


def get_sitePattern_map_matrix(taxaPerm, inverse=False):
    """
    ...
    relationship = get_sitePattern_relationship(taxaPerm, inverse=False)
    rows = np.arange(15)
...
def getTrueProbsQuartet(parameters, param_idx, isAsymm, taxaPerm, gamma_id):
    """
    Using the quartet features (param_idx, isAsymm, taxaPerm, gamma_id) of a combination of four taxa, get the true
    ...
    # For simplicity, we use these notation: pi = param_idx, ia = isAsymm, tp = taxaPerm, gi = gamma_id.
    for pi, ia, tp, gi in zip(param_idx,isAsymm,taxaPerm,gamma_id_clean):
        # 1. Get gamma weight
...
def get_n_D_hasAmbiguityCode(CIS_data, ACGT_weight=None, skip_gap=False, skip_missing=False):
...
def get_taxa_partition(CIS_data, phylox_network, Imap_file_path):
    ...
    # Returns True when CIS_taxon_name matches any one of the imap_names
    def matches(CIS_taxon_name, imap_names):
        for name in imap_names:
            # Replace space with underscore if there is any
            if ' ' in CIS_taxon_name:
                CIS_taxon_name = CIS_taxon_name.replace(" ", "_")
            # Remove caret ^ before taxon name
            if name == CIS_taxon_name.lstrip("^"):
                return True
        return False
...
def pair_quartetRows_quartetFeatures(CIS_data, phylox_network, major_tree, taxa_partition=None):
    """
    Given CIS_data and taxa_partition, we can select one individual per species to form a quartet matrix by
    choosing the rows (individuals) of CIS_data. The choice of row indices that forms quartet matrix is
    (quartet_rows). For each quartet_rows, we pair it up with the corresponding quartet features (param_idx,
    isAsymm, taxaPerm, gamma_id).
    ...
    if taxa_partition is None:
        taxa_partition = [
            # CIS_data row indices of the taxa that contains the species name in the network
            [idx for idx, taxon in enumerate(CIS_data.taxon_namespace) if label in taxon.label]
            for label in get_taxa_label(phylox_network)
        ]
    ...
    dict_quartet_isAsymm = {}
    dict_quartet_taxaPerm = {}
    dict_quartet_gamma_id = {}
    for MAP in all_1indivPerSpecies_maps:
        # For each all_1indivPerSpecies_maps, enumerate all possible combinations of 4 row indices of CIS_data
        # (quartet_rows).
        for j, quartet_rows in enumerate(combinations(MAP, 4)):
            ...
            dict_quartet_isAsymm[dict_key] = all_isAsymm[j]
            dict_quartet_taxaPerm[dict_key] = all_taxaPerm[j]
            dict_quartet_gamma_id[dict_key] = all_gamma_id[j]

    # Extract collapsed result from dictionary
    all_quartet_row_idx        = list(dict_quartet_param_idx.keys())
    all_quartet_param_idx      = list(dict_quartet_param_idx.values())
    all_quartet_isAsymm        = list(dict_quartet_isAsymm.values())
    all_quartet_taxaPerm      = list(dict_quartet_taxaPerm.values())
    all_quartet_gamma_id       = list(dict_quartet_gamma_id.values())

    return all_quartet_row_idx, all_quartet_param_idx, all_quartet_isAsymm, all_quartet_taxaPerm, all_quartet_gamma_id


def get_zippedData_net_reduced(CIS_data, phylox_network, major_tree,
    ...
    # -------------------------------------------------------------------------
    (all_quartet_row_idx, all_quartet_param_idx, all_quartet_isAsymm, all_quartet_taxaPerm,
     all_quartet_gamma_id) = pair_quartetRows_quartetFeatures(CIS_data, phylox_network, major_tree, taxa_partition)

    # -------------------------------------------------------------------------
    # 2. Compute global site-pattern count with ambiguity handling
    # -------------------------------------------------------------------------
    collapsed_IDcode, n_D = get_n_D_hasAmbiguityCode(CIS_data, ACGT_weight, skip_gap, skip_missing)
    ...
    def make_dict_key(param_idx, isAsymm, taxaPerm, gamma_id):
        """Make these quartet features as one dictionary key."""
        # reshape isAsymm into a column vector
        isAsymm_col = isAsymm[:, None]  # shape (2,1)

        # Stack row-wise: [param_idx | isAsymm | taxaPerm | gamma_id]
        combined = np.concatenate(
            [param_idx, isAsymm_col, taxaPerm, gamma_id],
            axis=1
        )

        return tuple(tuple(row) for row in combined)

    for row_idx, param_idx, isAsymm, taxaPerm, gamma_id in zip(all_quartet_row_idx, all_quartet_param_idx,
                                                all_quartet_isAsymm, all_quartet_taxaPerm, all_quartet_gamma_id):
        # Get the quartet site pattern array by the quartet row_idx
        ...
        dict_key = make_dict_key(param_idx, isAsymm, taxaPerm, gamma_id)
        ...
        dict_isAsymm[dict_key]      = isAsymm
        dict_taxaPerm[dict_key]     = taxaPerm
        dict_gamma_id[dict_key]     = gamma_id
    ...
    reduced_isAsymm             = list(dict_isAsymm.values())
    reduced_taxaPerm            = list(dict_taxaPerm.values())
    reduced_gamma_id            = list(dict_gamma_id.values())
    reduced_E_mat               = list(dict_E_mat.values())

    return (list(zip(reduced_SitePattern_count, reduced_param_idx, reduced_isAsymm, reduced_taxaPerm, reduced_gamma_id)),
            reduced_E_mat, n_D)
...
def compress_zippedData_net(zipped_data_net, all_E_mat, n_D):
    ...
    for (n_Q, param_idx, isAsymm, taxaPerm, gamma_id), E_mat in zip(zipped_data_net, all_E_mat):
...
def get_zippedData_net(CIS_data, phylox_network, major_tree,
                       taxa_partition=None, ACGT_weight=None, skip_gap=False, skip_missing=False):
    """
    ...
    for row_idx, param_idx, isAsymm, taxaPerm, gamma_id in zip(all_quartet_row_idx, all_quartet_param_idx,
                                                                all_quartet_isAsymm, all_quartet_taxaPerm,
        ...
        all_isAsymm.append(isAsymm)
        all_taxaPerm.append(taxaPerm)
    ...
    # -------------------------------------------------------------------------
    return list(zip(all_n_Q, all_param_idx, all_isAsymm, all_taxaPerm, all_gamma_id)), all_E_mat, n_D


def SpeciesNetwork_CompLogLik(zipped_data_net, parameters):
    """Computes species network composite log likelihood."""
    CompLogLik = 0
    for n_Q, param_idx, isAsymm, taxaPerm, gamma_id in zipped_data_net:
        TrueProbs = getTrueProbsQuartet(parameters, param_idx, isAsymm, taxaPerm, gamma_id)
...
def get_MCLE_parameters(zipped_data_net, phylox_network, gamma_parameters=None, is_fixed_param=None,
    ...
    precomputed_data = []
    for n_Q, param_idx, isAsymm, taxaPerm, gamma_id in zipped_data_net:
        precomputed_data.append((n_Q / n_Q.sum(), param_idx, isAsymm, taxaPerm))
    ...
    def get_MOM_tree_param(theta):
        ...
        for p_hat_Q, param_idx, isAsymm, taxaPerm in precomputed_data:
            for pi, ia, tp in zip(param_idx, isAsymm, taxaPerm):
...
def printQuartet(zipped_data_net, phylox_network, parameters):
    ...
    # Get all taxa_labels
    all_taxa_labels = combinations([str(label) for label in get_taxa_label(phylox_network)], 4)

    for taxa_labels, (n_Q, param_idx, isAsymm, taxaPerm, gamma_id) in zip(all_taxa_labels, zipped_data_net):
        p_Q = getTrueProbsQuartet(parameters, param_idx, isAsymm, taxaPerm, gamma_id)
...
def get_Grad_Hess_Quartet(parameters, n_Q, param_idx, isAsymm, taxaPerm, gamma_id):
    """Get the gradient vector and Hessian matrix of the quartet likelihood with quartet features (param_idx, isAsymm,
    taxaPerm, gamma_id) given parameters."""
    import itertools
    ...
    def Qt_1st_2nd_deriv_Symm(tree_parameters, gamma_parameters, pi, tp, gi):
        """(pi = param_idx, ia = isAsymm, tp = taxaPerm, gi = gamma_id)
    ...
    # Take derivatives of quartet (pi = param_idx, ia = isAsymm, tp = taxaPerm, gi = gamma_id)
    for pi, ia, tp, gi in zip(param_idx, isAsymm, taxaPerm, gamma_id_clean):
...
def get_Vari_Sens_Mat(parameters, zipped_data_net, all_E_mat, n_D):
    ...
    # -------------------------------------------------------------------------
    for (n_Q, param_idx, isAsymm, taxaPerm, gamma_id), E_Q_mat in zip(zipped_data_net, all_E_mat):
        # Get R_Q_mat, H_Q_mat from quartet Q
        R_Q_mat, H_Q_mat = get_Grad_Hess_Quartet(parameters, n_Q, param_idx, isAsymm, taxaPerm, gamma_id)
...
def get_tau_prior_and_constraint(phylox_network):
    ...
    clone_network = ladderize_network(clone_network, ascending=False)
